...
def simule(pdf: Path, ham: Path, hedef: Path, log_f: Path):
    """Geliştirme amaçlı benzetim (DONUSUM_SIMULE=1): marker yokken arayüz akışını denemek için.
    ...
    n = 1048; m = (n + 199) // 200
    bekle = float(os.environ.get("DONUSUM_SIMULE_SN", "1.5"))
...
def main():
    ...
    import os
    if os.environ.get("DONUSUM_SIMULE") == "1":
        if a.test:
